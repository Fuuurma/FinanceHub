...
import logging
from websockets import WebSocketException, ConnectionClosed, InvalidURI
...
class BinanceWebSocketClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.websocket = await websockets.connect(
                self.BASE_WS_URL, ping_interval=20, ping_timeout=60, close_timeout=10
            )
        ...
        except (WebSocketException, ConnectionClosed, InvalidURI, ValueError, KeyError) as e:
    ...
    async def connect_combined(self, streams: list) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Connecting to combined Binance WebSocket: {len(streams)} streams"
            )

            self.websocket = await websockets.connect(
                url, ping_interval=20, ping_timeout=60, close_timeout=10
            )
    ...
    async def subscribe_depth(
        self, symbol: str, level: int = 5, callback: Optional[Callable] = None
    ):
        """
    ...
    async def subscribe_depth_diff(
        self,
        symbol: str,
    ...
    async def listen_with_reconnect(self):
        ...
        while reconnect_count < self.max_reconnect_attempts:
            if not self.is_connected:
                logger.info(
                    f"Attempting to reconnect... (attempt {reconnect_count + 1})"
                )
