...
@api_endpoint(ttl=CACHE_TTLS['portfolio'], rate=RATE_LIMITS['analytics'], key_prefix="portfolio")
async def get_portfolio_summary(request, portfolio_id: str):
    ...
    total_pnl_percent = (Decimal(total_pnl) / Decimal(total_invested) * 100) if total_invested > 0 else Decimal('0')
...
@api_endpoint(ttl=CACHE_TTLS['analytics'], rate=RATE_LIMITS['analytics'], key_prefix="portfolio")
async def get_performance_metrics(
    ...
    total_return_percent = (Decimal(total_return) / Decimal(total_cost) * 100) if total_cost > 0 else Decimal('0')
...
async def get_portfolio_comparison(
    ...
    total_cost = sum(Decimal(h.purchase_price) * Decimal(h.quantity) for h in holdings if h.purchase_price and h.quantity)
    portfolio_return = (Decimal(total_value) - Decimal(total_cost)) / Decimal(total_cost) if total_cost > 0 else Decimal('0')
