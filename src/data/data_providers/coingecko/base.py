...
# apps/assets/services/fetchers/coinmarketcap_fetcher.py
...
class CoinMarketCapFetcher:
    """
    CoinMarketCap API Fetcher
    Basic: 333 calls/day, 10,000 calls/month
    Pro: Higher limits
    """
    ...
    async def _request(self, endpoint: str, params: Optional[Dict] = None) -> Dict:
        ...
        try:
            async with self.session.get(url, params=params) as response:
                ...
                # Rate limiting for free tier (333/day = ~1 per 4 minutes)
                await asyncio.sleep(0.5)

                return data

        except Exception as e:
            logger.error(f"CoinMarketCap API error: {str(e)}")
            raise
    ...
    async def get_cryptocurrency_listings_latest(
        ...
        Args:
            start: Start offset
            limit: Number of results (1-5000)
            convert: Currency to convert to (USD, EUR, etc.)
    ...
    async def get_cryptocurrency_quotes_historical(
        self,
        id: Optional[str] = None,
        symbol: Optional[str] = None,
        ...
        """
        Get historical market quotes

        Args:
            id: CoinMarketCap ID
            symbol: Ticker symbol
            time_start: Start time (ISO 8601 or UNIX timestamp)
            time_end: End time
        ...
        params = {"count": count, "interval": interval, "convert": convert}
        ...
        if id:
            ...
            raise ValueError("Must provide id or symbol")
    ...
    async def get_cryptocurrency_ohlcv_historical(
        self,
        id: Optional[str] = None,
        symbol: Optional[str] = None,
        time_start: Optional[str] = None,
        ...
        Args:
            interval: hourly, daily, weekly, monthly, yearly,
                     1h, 2h, 3h, 4h, 6h, 12h, 1d, 2d, 3d, 7d, 14d, 15d, 30d, 60d, 90d, 365d
        """
    ...
    async def get_exchange_map(self, listing_status: str = "active") -> Dict:
        """Get exchange ID map"""
        endpoint = "exchange/map"
        params = {"listing_status": listing_status}
        return await self._request(endpoint, params)

    async def get_fiat_map(self, include_metals: bool = False) -> Dict:
        """Get fiat currency mapping"""
        endpoint = "fiat/map"
        params = {"include_metals": str(include_metals).lower()}
        return await self._request(endpoint, params)
