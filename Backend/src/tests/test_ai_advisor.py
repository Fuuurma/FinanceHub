...
from utils.services.ai.ai_advisor import AIAdvisorService, AIResponse, UsageStats
...
class TestAIResponseGeneration:
    ...
    def test_explain_options_strategy(self, mock_call_llm, advisor_service, mock_paid_user):
    ...
    @patch.object(AIAdvisorService, '_call_llm')
    def test_suggest_options_strategy(self, mock_call_llm, advisor_service, mock_paid_user):
    ...
    @patch.object(AIAdvisorService, '_call_llm')
    def test_generate_forecast_narrative_arima(self, mock_call_llm, advisor_service, mock_paid_user):
    ...
    @patch.object(AIAdvisorService, '_call_llm')
    def test_generate_forecast_narrative_garch(self, mock_call_llm, advisor_service, mock_paid_user):
...
class TestAPIEndpoints:
    ...
    def test_access_status_unauthenticated(self, client):
    ...
    def test_explain_strategy_requires_auth(self, client):
        """Test that strategy explanation requires authentication."""
        response = client.post("/explain-strategy", {
            "strategy_type": "call",
            "underlying": "AAPL",
            "strike": 150.0,
            "expiry": "2024-03-15",
    ...
    def test_invalid_strategy_type(self, client, mock_paid_user):
    ...
    def test_invalid_model_type(self, client, mock_paid_user):
        """Test validation for invalid model type."""
        with patch('api.ai_advisor.check_ai_access') as mock_check:
            mock_check.return_value = None
            
            response = client.post("/forecast-narrative", {
                "symbol": "AAPL",
                "current_price": 150.0,
                "forecast_data": {},
                "model_type": "INVALID",
...
class TestAIAdvisorError:
    ...
    def test_llm_error_handling(self, mock_call_llm, advisor_service):
        ...
        import requests
        mock_call_llm.side_effect = requests.exceptions.ConnectionError("Proxy unavailable")
