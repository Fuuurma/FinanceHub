...
class IEXCloudScraper(BaseAPIFetcher):
    ...
    def extract_rate_limit_error(self, response: dict) -> Optional[str]:
        ...
        if response and isinstance(response, dict):
            if 'error' in response:
                ...
                if any(keyword in str(error).lower() for keyword in ['rate limit', 'too many', 'limit']):
                    return str(error)
    ...
    async def get_balance_sheet(
        ...
        return await self.request("stock/{symbol}/balance-sheet", params=params_with_key)
    ...
    async def get_news(
        ...
        return await self.request(f"stock/{symbol}/news/last/{limit}", params=params_with_key)
                    ...
                    for bar in historical_data:
                        try:
                            await asyncio.to_thread(
                                ...
                                asset=asset,
                                timestamp=datetime.strptime(bar['date'][:10], '%Y-%m-%d'),
    ...
    async def fetch_multiple_stocks(
        ...
        results = {}
        async with self:
            tasks = [self.fetch_and_save_stock(symbol, historical_years) for symbol in symbols]
        ...
        logger.info(f"IEX Cloud: {success_count}/{len(symbols)} stocks fetched successfully")
