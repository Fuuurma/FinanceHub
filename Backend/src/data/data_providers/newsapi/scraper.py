...
class NewsAPIScraper(BaseAPIFetcher):
    ...
    async def fetch_and_save_news(
        ...
        try:
            ...
            if category:
                ...
                data = await self.get_headlines_by_category(category, page=1, page_size=limit)
            else:
                # Use keyword search
            ...
            for article in data['articles']:
                try:
                    ...
                    await asyncio.to_thread(
                        ...
                        published_at=datetime.fromisoformat(article.get('publishedAt')) if article.get('publishedAt') else None,
    ...
    async def fetch_multiple_queries(
        ...
        logger.info(f"Fetched {total_articles} total articles across {len(queries)} queries")
...
async def fetch_financial_news(limit_per_query: int = 20) -> Dict[str, List]:
    ...
    logger.info(f"Fetched {sum(len(v) for v in all_articles.values())} total financial news articles")
    return all_articles
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print("Testing NewsAPI scraper with business news...")
        result = await scraper.fetch_and_save_news('business', category='business', limit=5)
        print(f"Business news Result: {result}")
        
        # Test with technology category
        print("\nTesting NewsAPI scraper with technology news...")
        result = await scraper.fetch_and_save_news('technology', category='technology', limit=5)
        print(f"Technology news Result: {result}")
        
        # Test with keyword search
        print("\nTesting NewsAPI scraper with keyword search...")
        result = await scraper.fetch_and_save_news('AAPL stock', limit=5)
        print(f"AAPL news Result: {result}")
        
        # Get top headlines
        print("\nGetting top headlines...")
        headlines = await scraper.get_top_headlines(country='us', page_size=10)
        print(f"Top headlines: {headlines}")
