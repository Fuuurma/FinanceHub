...
class DataProcessor:
    ...
    def detect_anomalies(prices: List[float]) -> List[int]:
        ...
        threshold = 3
        z_scores = np.abs((prices_array - mean) / std) if std > 0 else np.zeros_like(prices_array)
...
class PriceDataProcessor(DataProcessor):
    ...
    def normalize_price_data(self, raw_data: Dict[str, Any], source: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            normalized = {
                ...
                'close': self._parse_float(raw_data.get('close', raw_data.get('price', 0))),
    ...
    def process_historical_data(self, raw_data: List[Dict], source: str) -> List[Dict[str, Any]]:
...
class TechnicalIndicatorsCalculator:
    ...
    def calculate_all_indicators(self, price_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(price_data) < MIN_DATA_POINTS:
            logger.warning(f"Not enough data points for technical indicators: {len(price_data)}")
            return {}
    ...
    def _calculate_macd(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        # Calculate EMAs
        ema_fast = self._calculate_ema_series(close_series.to_list(), 2 / (MACD_FAST + 1))
        ema_slow = self._calculate_ema_series(close_series.to_list(), 2 / (MACD_SLOW + 1))
        ...
        return {
            ...
            'macd_cross': 'bullish' if histogram[-2] < 0 and histogram[-1] > 0 else 'bearish' if histogram[-2] > 0 and histogram[-1] < 0 else 'none',
    ...
    def _calculate_bollinger_bands(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'squeeze': bandwidth.to_list()[last_idx] < 0.02,  # Narrow bands indicate squeeze
            'status': 'overbought' if percent_b.to_list()[last_idx] > 80 else 'oversold' if percent_b.to_list()[last_idx] < 20 else 'neutral'
        }
    ...
    def _calculate_support_resistance(self, df: pl.DataFrame, lookback: int = 50) -> Dict[str, Any]:
        ...
        # Simple support/resistance using pivots
        pivot_high = max(highs)
        pivot_low = min(lows)
        pivot_point = (pivot_high + pivot_low + recent['close'].to_list()[-1]) / 3
        
        # Calculate support levels (multiple lows)
        support_levels = sorted(set(lows))[:5]
        
        # Calculate resistance levels (multiple highs)
        resistance_levels = sorted(set(highs), reverse=True)[:5]
        ...
        return {
            ...
            'current_support': support_levels[0] if support_levels else None,
            'current_resistance': resistance_levels[0] if resistance_levels else None,
    ...
    def process_raw_data(
        ...
        try:
            ...
            if asset_type in ['stock', 'crypto']:
                processed.normalized_data = self.price_processor.normalize_price_data(raw_data, source)
