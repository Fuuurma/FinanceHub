        ...
        for symbol in symbols:
            try:
                ...
                await cache_manager.set(
                    "crypto_price", symbol, value=price_data, ttl=300
                )
    ...
    try:
        ...
        for symbol in symbols:
            try:
                ...
                await cache_manager.set(
                    "stock_price", symbol, value=price_data, ttl=900
                )
...
async def fetch_crypto_historical_task(symbol: str, days: int = 30):
    ...
    try:
        ...
        await cache_manager.set(
            "crypto_historical", symbol, value=historical_data, ttl=3600
        )
...
@dramatiq.actor
async def fetch_stock_historical_task(
    symbol: str, timespan: str = "day", days: int = 30
):
...
async def fetch_news_task(query: str = None, category: str = None, count: int = 50):
    ...
    try:
        ...
        news_data = await newsapi.search_news(
            query=query, category=category, page_size=count
        )

        await cache_manager.set("news", "general", value=news_data, ttl=3600)
...
async def fetch_technical_indicators_task(symbol: str, indicators: List[str] = None):
    ...
    try:
        ...
        await cache_manager.set("technical_indicators", symbol, value=results, ttl=300)
...
async def cache_warming_task(symbols: List[str] = None):
    ...
    try:
        ...
        logger.info(
            f"Cache warming completed: {warmed}/{len(symbols) * len(data_types)} entries"
        )
...
def schedule_tasks():
    ...
    fetch_stock_prices_task.send_with_options(
        queue="high_priority", kwargs={"count": 20}
    )
