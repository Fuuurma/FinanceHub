...
async def fetch_crypto_batch(
    ...
    try:
        ...
        logger.info(f"Fetching crypto batch for {len(symbols)} symbols (validation={use_validation})")
        ...
        return {
            ...
            'provider_summary': {
                ...
                'coingecko_healthy': provider_summary['provider_health']['coingecko']['is_healthy'],
                'coinmarketcap_healthy': provider_summary['provider_health']['coinmarketcap']['is_healthy']
            },
...
async def fetch_crypto_quotes(
    ...
    try:
        ...
        if data:
            logger.info(f"Successfully fetched quote for {symbol}: ${data.get('price', 0)}")
        else:
...
async def validate_crypto_batch(symbols: List[str]) -> dict:
    ...
    try:
        ...
        total = len(results)
        avg_confidence = sum(
            r.overall_confidence for r in results.values() if r.overall_confidence
...
async def detect_crypto_anomalies(
    ...
    try:
        ...
        logger.info(f"Checking {len(symbols)} symbols for anomalies (threshold={threshold})")
...
async def periodic_crypto_update() -> dict:
    ...
    try:
        ...
        logger.info(f"Periodic update complete: {result.get('success', 0)}/{result.get('total', 0)} successful")
