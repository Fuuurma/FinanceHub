...
class VaRRequest(BaseModel):
    ...
    risk_free_rate: float = Field(
        default=0.03, ge=0, description="Annualized risk-free rate"
    )
    confidence_level: float = Field(
        default=0.95, ge=0.9, le=0.99, description="Confidence level"
    )
...
class ComprehensiveRiskAnalysisRequest(BaseModel):
    ...
    stress_scenarios: Optional[List[Dict[str, float]]] = Field(
        None, description="Stress scenarios"
    )


@router.post("/var", response=VaRResponse)
@api_endpoint(
    ttl=CACHE_TTLS["short"], rate=RATE_LIMITS["data_intensive"], key_prefix="risk"
)
def calculate_var(request, data: VaRRequest):
    ...
    result = risk_manager.monte_carlo_var(
        confidence_levels=[data.confidence_level], n_simulations=data.n_simulations
    )
...
def stress_test(request, data: StressTestRequest):
    ...
    if len(data.scenarios) == 0:
        raise ValidationException(
            "At least one stress scenario required", {"min_scenarios": 1}
        )
...
def get_expected_shortfall(
    ...
    n_simulations: int = Query(
        default=5000, ge=1000, le=50000, description="Number of simulations"
    ),
):
