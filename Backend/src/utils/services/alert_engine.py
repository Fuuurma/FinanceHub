...
class AlertEngine:
    ...
    async def _check_price_alert(self, alert: Alert) -> bool:
        try:
            response = await self.orchestrator.get_market_data(
                data_type='crypto_price' if alert.symbol.upper() in ['BTC', 'ETH', 'SOL'] else 'stock_price',
                symbol=alert.symbol
            )
            ...
            price = float(response.data.get('price', 0))
    ...
    async def _check_percentage_alert(self, alert: Alert) -> bool:
        try:
            response = await self.orchestrator.get_market_data(
                data_type='crypto_historical' if alert.symbol.upper() in ['BTC', 'ETH', 'SOL'] else 'stock_historical',
                symbol=alert.symbol,
                params={'days': 2}
            )
            ...
            previous_price = float(data[-2].get('close', 0))
    ...
    async def _check_macd_alert(self, alert: Alert) -> bool:
        try:
            ...
            signal_val = float(current['signal'])
    ...
    async def _check_volume_alert(self, alert: Alert) -> bool:
        try:
            ...
            volumes = [float(d.get('volume', 0)) for d in data[-14:]]
            avg_volume = sum(volumes) / len(volumes)
            current_volume = volumes[-1]
            
            threshold_multiplier = float(alert.condition_value)
    ...
    async def _send_websocket_notification(self, history: AlertHistory, notification: AlertNotification):
        try:
    ...
    async def _send_email_notification(self, history: AlertHistory, notification: AlertNotification):
        try:
...
This is an automated message from FinanceHub.
    ...
    async def _send_push_notification(self, history: AlertHistory, notification: AlertNotification):
        try:
            notification.metadata = {
    ...
    def get_alert_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        alert_type_counts = Alert.objects.filter(
            user_id=user_id
