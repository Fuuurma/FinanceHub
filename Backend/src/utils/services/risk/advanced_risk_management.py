...
class RiskManager:
    ...
    def __init__(self, returns: np.ndarray, weights: np.ndarray, risk_free_rate: float = 0.03):
        """
        ...
        self.portfolio_returns = np.sum(self.returns * self.weights[:, np.newaxis], axis=0)
    ...
    def monte_carlo_var(
        ...
        cumulative_returns = np.cumprod(1 + portfolio_simulated[:, :time_horizon], axis=1)
    ...
    def factor_stress_test(
        ...
        # Find worst factor
        worst_factor = max(portfolio_impacts.keys(), key=lambda k: abs(portfolio_impacts[k]))
...
def get_risk_manager(returns: np.ndarray, weights: np.ndarray, risk_free_rate: float = 0.03) -> RiskManager:
    """
