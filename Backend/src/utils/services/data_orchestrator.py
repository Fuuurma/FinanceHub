...
class DataOrchestrator:
    ...
    async def get_market_data(
        ...
        params = params or {}
        freshness_required = self.freshness_requirements.get(data_type, DataFreshness.RECENT)
        ...
        if request_id in self.active_requests:
            ...
            return await self.get_market_data(data_type, symbol, params, force_refresh, priority)
        
        self.active_requests.add(request_id)
        
        try:
            if not force_refresh:
                cached_response = await self._try_cache(data_type, symbol, params, freshness_required)
                
                if cached_response:
                    self._record_request(request_id, DataSource.CACHE, True)
                    return cached_response
            
            live_response = await self._fetch_live_data(data_type, symbol, params, priority)
    ...
    async def _try_cache(
        ...
        try:
            ...
            if isinstance(cached_data, dict) and 'timestamp' in cached_data:
                cache_age = (timezone.now() - timezone.make_aware(
    ...
    async def _fetch_crypto_data(
        ...
        try:
            ...
            return DataResponse(
                ...
                source=source,
                cached=False,
                freshness=DataFreshness.REALTIME if data_type == 'crypto_price' else DataFreshness.CACHED
            )
    ...
    async def _fetch_stock_data(
        ...
        try:
            ...
            if data_type == 'stock_price':
                data = await scraper.get_aggregate_bars(symbol, timespan='day', multiplier=1, days=1)
            ...
            return DataResponse(
                ...
                source=data_source,
                cached=False,
                freshness=DataFreshness.RECENT if data_type == 'stock_price' else DataFreshness.CACHED
            )
    ...
    async def _fetch_realtime_data(
        ...
        raise NotImplementedError("Real-time data (order book, trades) requires WebSocket connection")
