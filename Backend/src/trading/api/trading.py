...
def create_order(request, payload: OrderCreateIn):
    ...
    if payload.order_type in [OrderType.LIMIT, OrderType.STOP, OrderType.STOP_LIMIT] and not payload.price:
        from ninja.errors import HttpError
        raise HttpError(400, "Price is required for limit, stop, and stop-limit orders")
    
    if payload.order_type in [OrderType.STOP, OrderType.STOP_LIMIT] and not payload.stop_price:
        from ninja.errors import HttpError
    ...
    if payload.order_type == OrderType.OCO and payload.oco_linked_order_id:
        order.oc o_linked_order_id = payload.oco_linked_order_id
...
def cancel_order(request, order_id: str):
    ...
    if not order.is_active:
        from ninja.errors import HttpError
        raise HttpError(400, "Cannot cancel orders that are already filled or cancelled")
    
    # Cancel linked OCO order if exists
    if order.oc o_linked_order:
        try:
            linked_order = Order.objects.get(id=order.oc o_linked_order_id)
    ...
    for pos in positions:
        try:
            ...
            if pos.current_price and pos.is_open:
                ...
                pos.unrealized_pnl_percent = (pos.unrealized_pnl / pos.cost_basis) * Decimal('100') if pos.cost_basis > 0 else Decimal('0')
...
def close_position(request, position_id: str):
    ...
    position.unrealized_pnl_percent = (position.unrealized_pnl / position.cost_basis) * Decimal('100') if position.cost_basis > 0 else Decimal('0')
...
def get_account_summary(request):
    ...
    day_trades_count = 0
    for order in Order.objects.filter(user=user, status=OrderStatus.FILLED, created_at__date=today):
...
def get_positions_summary(
    ...
    today = datetime.now().date()
    today_pnl = sum(p.realized_pnl for p in positions if p.closed_at and p.closed_at.date() == today)
